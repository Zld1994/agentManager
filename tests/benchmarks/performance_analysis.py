...
class PerformanceAnalyzer:
    ...
    def _analyze_throughput(self, metric: Dict[str, Any], test_name: str):
        ...
        if current == 0:
            self.insights.append(PerformanceInsight(
                ...
                recommendation="Investigate test execution and ensure tasks are being processed"
            ))
            return
        ...
        if baseline:
            ...
            if deviation < -20:
                ...
                self.insights.append(PerformanceInsight(
                    ...
                    baseline_value=baseline.throughput,
                    deviation_percent=deviation,
                    recommendation="Monitor performance trends and investigate potential bottlenecks"
                ))
            elif deviation > 20:
                self.insights.append(PerformanceInsight(
                    category='throughput',
    ...
    def _analyze_latency(self, metric: Dict[str, Any], test_name: str):
        ...
        if current_p95 > 1000:
            self.insights.append(PerformanceInsight(
                ...
                deviation_percent=None,
                recommendation="Profile code to identify slow operations and optimize critical paths"
            ))
        ...
        if current_p99 > 2000:
            self.insights.append(PerformanceInsight(
                ...
                deviation_percent=None,
                recommendation="Investigate tail latency causes and implement caching or optimization"
            ))
    ...
    def _analyze_memory(self, metric: Dict[str, Any], test_name: str):
        ...
        current_avg = metric.get('memory_avg', 0)
        baseline = self.baselines.get(test_name)
        ...
        if current_peak > 1000:
            self.insights.append(PerformanceInsight(
                ...
                deviation_percent=None,
                recommendation="Review memory allocation patterns and consider streaming or pagination"
            ))
    ...
    def _analyze_cpu(self, metric: Dict[str, Any], test_name: str):
        ...
        current_avg = metric.get('cpu_avg', 0)
        baseline = self.baselines.get(test_name)
    ...
    def identify_bottlenecks(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if metrics:
            ...
            sorted_by_throughput = sorted(
                metrics,
                key=lambda m: m.get('throughput', 0)
            )
            ...
            sorted_by_latency = sorted(
                metrics,
                key=lambda m: m.get('latency_p95', 0),
                reverse=True
            )
            ...
            sorted_by_memory = sorted(
                metrics,
                key=lambda m: m.get('memory_peak', 0),
                reverse=True
            )
            bottlenecks['memory_bottlenecks'] = [
                {
                    'test': m.get('test_name'),
                    'memory_peak': m.get('memory_peak', 0),
                    'rank': i + 1
                }
            ...
            sorted_by_cpu = sorted(
                metrics,
                key=lambda m: m.get('cpu_peak', 0),
                reverse=True
            )
            bottlenecks['cpu_bottlenecks'] = [
                {
                    'test': m.get('test_name'),
                    'cpu_peak': m.get('cpu_peak', 0),
                    'rank': i + 1
                }
            ...
            sorted_by_errors = sorted(
                metrics,
                key=lambda m: m.get('error_rate', 0),
                reverse=True
            )
    ...
    def compare_with_baseline(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for metric in metrics:
            ...
            is_regression = (
                throughput_deviation < -10 or
                latency_deviation > 20 or
                memory_deviation > 30
            )
            
            is_improvement = (
                throughput_deviation > 20 or
                latency_deviation < -20 or
                memory_deviation < -20
            )
